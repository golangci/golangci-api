...
func authHandler(w http.ResponseWriter, r *http.Request) {
	q := r.URL.Query()
	ru := fmt.Sprintf("%s?state=%s&code=test", q.Get("redirect_uri"), q.Get("state"))
	http.Redirect(w, r, ru, http.StatusTemporaryRedirect)
}

func sendFakeGithubJSON(name string, w http.ResponseWriter) {
	w.Header().Add("Content-Type", "application/json")
	f, err := os.Open(path.Join(fsutil.GetProjectRoot(), "test", "data", "github_fake_response", name+".json"))
	if err != nil {
